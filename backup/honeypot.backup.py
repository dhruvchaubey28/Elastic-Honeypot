...
async def geolocate_ip(ip: str) -> dict | None:
    ...
    try:
        ...
        resp = await client.fetch(url, raise_error=False, connect_timeout=3.0, request_timeout=5.0)
...
async def log_event(source_ip, event_type, request_uri, user_agent,
                    method="GET", post_body=None) -> dict | None:
...
# ───────────────────────────────────────────
#  Email alert (fully async)
# ───────────────────────────────────────────

async def send_alert(source_ip, request_uri, user_agent, geo,
...
class FakeElasticsearchHandler(BaseHandler):
    async def get(self):
        ...
        self.write(json.dumps({
            ...
            "version": {"number": "7.10.0", "build_flavor": "default", "build_type": "docker"},
...
class FakeLoginHandler(tornado.web.RequestHandler):
    ...
    async def post(self):
        ...
        await log_event(ip, "Credential Submission", self.request.uri, ua, "POST", f"user={username}")
...
class HealthHandler(tornado.web.RequestHandler):
    async def get(self):
        ...
        try:
            ...
            self.write({"status": "ok", "service": "honeypot", "db": "connected"})
        except Exception as e:
            self.set_status(503)
...
async def main():
    ...
    logger.info(f"🍯 Delilah Honeypot started on port {HONEYPOT_PORT}")
    logger.info(f"📊 Geo cache TTL: {GEO_CACHE_TTL}s")
